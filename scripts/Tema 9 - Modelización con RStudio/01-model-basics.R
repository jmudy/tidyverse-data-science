...
# y~x <-> y = a0 + a1x <-> y = a_0*out0 + a1*out1
...
# y ~ sex <-> y = a0 + a1*sexmale(sex_male = 0,1)
df <- tribble(
  ~sex,    ~value,
  "male",       1,
  "female",     5,
...
tibble(x = "e") %>% # Aquí falla porque no hay "e" en el grid
  add_predictions(mod2)
...
mod2 <- lm(y ~ x1 * x2, data = sim3) # con interacciones
...
# explicación parámetro "pretty" de lafunción seq_range()
...
seq_range(x1, n = 10)
seq_range(x1, n = 10, trim = 0.10)
seq_range(x1, n = 10, trim = 0.25)
seq_range(x1, n = 10, trim = 0.50)

x2 <- c(0,1)
seq_range(x2, n = 10)
seq_range(x2, n = 10, expand = 0.10) # 10% repartido por abajo y por arriba
seq_range(x2, n = 10, expand = 0.25)
seq_range(x2, n = 10, expand = 0.50)

#############################

grid %>% ggplot(aes(x1, x2)) +
...
nobs(mod) # indica 3 porque hay NAs en 2 observaciones
